...
def load_data():
	...
	df = pd.DataFrame({ "categories": categories,"datas": datas})
	# selected = ['categories', 'datas"']
	df = df.reindex(np.random.permutation(df.index))
	labels = sorted(list(set(df['categories'].tolist())))
	num_labels = len(labels)
	one_hot = np.zeros((num_labels, num_labels), int)
	np.fill_diagonal(one_hot, 1)
	label_dict = dict(zip(labels, one_hot))

	x_raw= df['datas'].apply(lambda x: clean_str(x).split(' ')).tolist()
	y_raw = df['categories'].apply(lambda y: label_dict[y]).tolist()
